...
app.post("/projects/:id/actions", authorized, async (request, response) => { const parsed = z.object({ action: z.enum(["start", "stop", "restart"]) }).safeParse(request.body); if (!parsed.success) return response.status(400).json({ error: "Invalid action." }); try { await projectAction(request.params.id, parsed.data.action); return response.json({ ok: true }); } catch (error) { return response.status(409).json({ error: error instanceof Error ? error.message : "Runner action failed." }); } });
